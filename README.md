...
graph TD
    subgraph "Инициализация"
        ...
        C --> D["Запуск параллельных потоков"];
    end
    ...
    S --> E
    S --> G
    S --> Q{"Есть сигнал завершения?"};
    Q -->|Да| L["Освобождение камеры, Закрытие окна"];
	Q -->|Нет| S;
...
Для быстрой подготовки и запуска проекта используйте один из скриптов в корне:
...
Эти скрипты автоматически создадут виртуальное окружение, установят зависимости и скопируют настройки из `.env.example` в `.env` (если он ещё не создан).

Для запуска вручную выполните главный скрипт:
